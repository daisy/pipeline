 ...
 <!-- Run on 'document.xml' inside a .docx -->
  ...
  <xsl:template match="w:p">
    <p>
      ...
      <!-- Ignoring properties on the paragraph itself ... -->
...
<!-- mode 'build-properties' can be called on any w:pPr or w:rPr (paragraph or run properties element)
  ...
  <xsl:template mode="build-properties"  as="element(xsw:prop)" match="w:b[not(@w:val=('0','none'))]">
    <xsw:prop name="font-weight">bold</xsw:prop>
  </xsl:template>

  <!-- will become 'i' -->
  <xsl:template mode="build-properties"  as="element(xsw:prop)" match="w:i[not(@w:val=('0','none'))]">
    <xsw:prop name="font-style">italic</xsw:prop>
  </xsl:template>

  <!-- will become 'u' -->
  <xsl:template mode="build-properties"  as="element(xsw:prop)" match="w:u[not(@w:val=('0','none'))]">
