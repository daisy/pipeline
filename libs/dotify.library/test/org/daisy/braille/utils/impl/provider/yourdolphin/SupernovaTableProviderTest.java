...
public class SupernovaTableProviderTest {
    ...
    public void testListLength() {
        assertEquals(
            "Assert that all tables have tests by counting the list length",
            1,
            bt.list().size()
        );
    }
    ...
    public void testTableSV_SE_6DOT() {
        ...
        assertEquals(
            ...
            " a,b.k;l@cif/msp'e:h*o!rødjgäntq_å?ê-u(v`îöë§xèç\"û+ü)z=àœôwï#yùé",
            text
        );
        assertTrue(
            "Assert that table does not support 8-dot",
            !ta.supportsEightDot()
        );
        assertEquals(
