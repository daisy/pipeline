...
public class PEFFileSplitterTest {
    ...
    public void testSplitter() throws IOException, TransformerException {
        ...
        PEFFileSplitter splitter = new PEFFileSplitter(t -> true);
        ...
        for (File v : res) {
            assertTrue(
                "Assert that file " + i + " begins with the string 'volume-'",
                v.getName().startsWith("volume-")
            );
            assertTrue(
                "Assert that file " + i + " ends with the string '.pef'",
                v.getName().endsWith(".pef")
            );
            boolean equal = fc.compareXML(
                new FileInputStream(v),
                this.getClass().getResourceAsStream("resource-files/PEFFileSplitterTestExpected-" + i + ".pef")
            );
            assertTrue(
                "Assert that contents of file " + i + " is as expected: " + fc.getFileOne() +
                " vs. " + fc.getFileTwo() + " differs at byte position " + fc.getPos(),
                equal
            );
