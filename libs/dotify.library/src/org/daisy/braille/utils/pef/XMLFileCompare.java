...
public class XMLFileCompare extends FileCompare {
    private static final Logger logger = Logger.getLogger(XMLFileCompare.class.getCanonicalName());
    ...
    private static Templates init(TransformerFactory factory) {
        try {
            ...
            logger.throwing("XMLFileCompare", iae.getMessage(), iae);
        }
        try {
            return factory.newTemplates(
                new StreamSource(XMLFileCompare.class.getResourceAsStream("resource-files/normalize.xsl"))
            );
