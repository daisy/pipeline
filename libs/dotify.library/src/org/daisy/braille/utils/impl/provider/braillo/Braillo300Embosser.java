    ...
    public EmbosserWriter newEmbosserWriter(OutputStream os) {
        try {
            ...
            SimpleEmbosserProperties ep = SimpleEmbosserProperties.with(
                    Math.min(
                        EmbosserTools.getWidth(printPage, getCellWidth()), 42),
                        EmbosserTools.getHeight(printPage, getCellHeight()
                    )
            ).supportsDuplex(true).supportsAligning(true).build();
    ...
    private byte[] getBrailloHeader(int width, Dimensions pageFormat) throws UnsupportedPaperException {
        ...
        if (width > 330 || height > 14) {
            throw new UnsupportedPaperException(
                HEADER_TOO_WIDE_HIGH_MSG + width + HEADER_CHARS_MSG + height / 2d + HEADER_INCHES_MSG
            );
        }
        if (width < 140 || height < 4) {
            throw new UnsupportedPaperException(
                HEADER_TOO_NARROW_SHORT_MSG + width + HEADER_CHARS_MSG + height / 2d + HEADER_INCHES_MSG
            );
        }
