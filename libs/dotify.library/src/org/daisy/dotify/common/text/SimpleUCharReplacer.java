...
public class SimpleUCharReplacer extends HashMap<Integer, String> {
    private static final Logger logger = Logger.getLogger(SimpleUCharReplacer.class.getCanonicalName());
        ...
        for (Iterator<?> it = keys.iterator(); it.hasNext(); ) {
            ...
            if (key.codePointCount(0, key.length()) == 1) {
                ...
                try {
                    ...
                    logger.severe(
                        "error in translation table " + tableURL.toString() +
                        ": attribute key=\"" + key + "\" is not a hex number."
                    );
                }
            }
        }
