            ...
            for (FallbackRule r : fallbackRules) {
                if (!str.add(r.applyToCollection())) {
                    Logger.getLogger(this.getClass().getCanonicalName()).warning(
                        "Multiple rules for the same collection: " + r.applyToCollection()
                    );
                }
                if (collectionId.equals(r.applyToCollection())) {
                    ...
                    throw new IllegalArgumentException(
                        "This rule (" + r + ") can only be applied to the collection with id: " + collectionId
                    );
                }
            }
            if (!found) {
                throw new IllegalArgumentException(
                    "The fallback rules must include a rule for the collection that triggered the fallback action " +
                    "(" + collectionId + ")"
                );
            }
        }
