		...
		<xsl:result-document href="{concat($module,'/',$OUTPUT_FILENAME)}" method="text">
			...
			<xsl:if test="not($is-aggregator)">
				...
				<!--
				    ...
				    installed in order to make the projects build in Eclipse; the correct working of
				    the Makefile relies on Make putting this whole "install" part before the
				    "eclipse" part in the execution order, if this is not satisfied the Eclipse
				    projects are not linked up correctly
				-->
				<xsl:value-of select="concat(' ',$dirname,'.dependencies')"/>
				<xsl:text> \&#x0A;&#x09;</xsl:text>
				...
				<xsl:text> : \</xsl:text>
				<!--
				    dependencies always need to be regerenated together with dependent, otherwise
				    inter-module links are not made
				-->
				<xsl:variable name="dependencies" as="xs:string*">
					<xsl:for-each select="$artifacts-and-dependencies/self::pom:dependency[not(@fromDependencyManagement)
					                                                                       or @scope='import']">
