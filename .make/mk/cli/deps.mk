...
$(MVN_LOCAL_REPOSITORY)/org/daisy/pipeline/cli/2.4.0/cli-2.4.0.pom : cli/.install.pom | .group-eval
	+$(EVAL) if (new File("$@").exists()) touch("$@"); else exit(1);

$(MVN_LOCAL_REPOSITORY)/org/daisy/pipeline/cli/2.4.0/cli-2.4.0% : cli/.install% | .group-eval
	+$(EVAL) if (new File("$@").exists()) touch("$@"); else exit(1);
...
cli/.install.pom : %/.install.pom : | %/.compile-dependencies %/.test-dependencies

.SECONDARY : cli/.install-doc
cli/.install-doc : | .maven-init .group-eval
	+$(EVAL) mvn.installDoc("$(patsubst %/,%,$(dir $@))");

cli/.install-doc : %/.install-doc : | %/.compile-dependencies %/.test-dependencies
