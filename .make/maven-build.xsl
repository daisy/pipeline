...
<xsl:stylesheet version="1.0"
	...
	<xsl:template name="main">
		...
		<xsl:choose>
			...
			<xsl:otherwise>
				...
				<xsl:if test="not($SRC_DIR='')">
					<xsl:value-of select="concat(' ',$SRC_DIR)"/>
				</xsl:if>
				...
				<xsl:if test="not($MAIN_DIR='')">
					<xsl:value-of select="concat(' ',$MAIN_DIR)"/>
				</xsl:if>
				<xsl:text> %/.dependencies</xsl:text>
				<xsl:text>&#x0A;</xsl:text>
				<xsl:text>&#x0A;</xsl:text>
				...
				<xsl:for-each select="$module-pom/pom:project/pom:dependencyManagement/pom:dependencies/pom:dependency[pom:scope='import']">
					...
					<xsl:if test="substring($dependency-version, string-length($dependency-version) - string-length('-SNAPSHOT') + 1) = '-SNAPSHOT'">
						<xsl:value-of select="concat($dirname,'.dependencies : ')"/>
						<xsl:call-template name="location-in-repo">
	...
	<xsl:template match="pom:project">
		...
		<xsl:for-each select="pom:parent|
			...
			<xsl:choose>
				<xsl:when test="substring($version, string-length($version) - string-length('-SNAPSHOT') + 1) = '-SNAPSHOT'">
					<xsl:value-of select="concat($dirname,'.dependencies : ')"/>
