...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    <xsl:template match="/">
        <xsl:message>Move out inlined definition lists</xsl:message>
        <xsl:apply-templates/>
    </xsl:template>
    
    <xsl:function name="f:is-target" as="xs:boolean">
        <xsl:param name="elem" as="item()*"/>
        <xsl:sequence select="exists($elem/self::dtb:dl)"/>
    </xsl:function>
