...
<!--
  <xsl:template match="*">
    <ssml:speak version="1.0">
      <xsl:choose>
        <xsl:when test="local-name()='speak'">
...
<!--
  <xsl:template match="*[@alt and not(*) and not(.//text())]" mode="copy">
    < !- - for unknown empty tags with an "alt" element, take the alt element instead of the tag - ->
