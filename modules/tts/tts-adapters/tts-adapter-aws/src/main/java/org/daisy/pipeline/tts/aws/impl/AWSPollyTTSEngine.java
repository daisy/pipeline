...
public class AWSPollyTTSEngine extends TTSEngine {
	 ...
	 */
	public AWSPollyTTSEngine(AWSPollyTTSService service, String accessKey, String secretKey, String region, int priority) throws SynthesisException {
		super(service);
	...
	/*private static final Pattern ssmlPrefix = compile("<(/?)ssml:", CASE_INSENSITIVE);
	private static final Pattern ssmlNamespace = compile("xmlns:ssml", CASE_INSENSITIVE);
    public static String normalizeSSMLToPolly(String sentence) {
    	...
    	String result = ssmlNamespace.matcher(
			ssmlPrefix.matcher(sentence).replaceAll("<$1")
		).replaceAll("xmlns");
