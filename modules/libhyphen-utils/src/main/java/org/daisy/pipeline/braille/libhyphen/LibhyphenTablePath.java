...
public class LibhyphenTablePath extends BundledResourcePath implements LibhyphenTableProvider {
	...
	private Provider<Locale,URI> provider = new Provider.util.VaryLocale<Locale,URI>() {
		public Iterable<URI> _get(Locale locale) {
			...
			URI resource = null;
			if (!"".equals(variant))
				resource = URLs.asURI(String.format("hyph_%s_%s_%s.dic", language, country, variant));
			else if (!"".equals(country))
				resource = URLs.asURI(String.format("hyph_%s_%s.dic", language, country));
			else
				resource = URLs.asURI(String.format("hyph_%s.dic", language));
