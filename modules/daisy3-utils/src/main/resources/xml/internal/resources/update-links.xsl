...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" version="2.0"
                ...
                xmlns="http://www.daisy.org/z3986/2005/resource/"
                xpath-default-namespace="http://www.daisy.org/z3986/2005/resource/"
    ...
    <xsl:template match="img/@src            |
        ...
        <xsl:variable name="new-file" as="element(d:file)?" select="(if (exists($fragment))
                                                                       then $new-file[d:anchor[(@original-id,@id)=$fragment]]
                                                                       else(),
                                                                     $new-file)[1]"/>
        <xsl:choose>
