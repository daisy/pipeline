...
- idref must be present on noteref and annoref. Add idref if missing or change if empty.
  ...
  The value of the idref must include a fragment identifier. Add a hash mark in the beginning of all
  idref attributes that don't contain a hash mark.
...
- Update the @page attribute to make it match the contents of the pagenum element.
  ...
  If @page="normal" but the contents of the element doesn't match "normal" content, the @page
  attribute is changed to:
  - @page="front" if the contents is roman numerals and the pagenum element is located in the frontmatter of the book
  - @page="special" otherwise
  
  If @page="front" but the contents of the element doesn't match "front" content (neither roman nor
  arabic numerals), the @page attribute is changed to "special"
...
from em, strong, sub, sup, pagenum, noteref.</p>
    ...
    <p:option name="narrator" select="'false'" px:type="boolean">
        <p:documentation xmlns="http://www.w3.org/1999/xhtml">
            ...
            <div px:role="desc" xml:space="preserve">Apply "Pipeline 1 narrator" cleaning routines on the document
