					...
					<xsl:variable name="date" as="xs:string?">
						<xsl:choose>
							...
							<xsl:when test="some $src in dc:source satisfies meta[@property='dcterms:date']
								...
								<xsl:sequence select="(for $src in dc:source return
							...
							<xsl:when test="$dtbook-metadata//dtb:meta[lower-case(@name)='dtb:sourceDate']/string(@content)">
								<xsl:sequence select="$dtbook-metadata//dtb:meta[lower-case(@name)='dc:sourceDate']
								                      /string(@content)[1]"/>
							</xsl:when>
							...
							<xsl:when test="not($dtbook-metadata//dtb:meta[lower-case(@name)='dc:source']) and
								...
								<xsl:sequence select="$dtbook-metadata//dtb:meta[lower-case(@name)='dc:date']
								                      /string(@content)[1]"/>
							</xsl:when>
						</xsl:choose>
					</xsl:variable>
