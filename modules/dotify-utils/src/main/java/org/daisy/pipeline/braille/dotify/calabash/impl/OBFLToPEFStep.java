	...
	public void run() throws SaxonApiException {
		...
		try {
			...
			if ("text/css".equals(styleType)) {
				...
				if (locale != null && !"und".equals(locale) && !mainQuery.containsKey("document-locale")) {
					Locale l = parseLocale(locale);
					if ("Brai".equals(l.getScript())) {
						// Even when the document has previously been pre-transcribed, we choose to
						// not include the "Brai" subtag for selecting the default translator, so
						// that the default translator is useful for handling untranscribed portions
						// of the document, or text generated by Dotify (including anything in the page
						// header/footer).
						// Note that in BrailleTranslatorFactoryServiceImpl (i.e. when selecting
						// translators based on "translate" and "xml:lang" attributes), we force
						// selection of PreTranslatedBrailleTranslator in case of a "Brai" subtag.
						locale = locale.replace("-Brai", "");
					}
