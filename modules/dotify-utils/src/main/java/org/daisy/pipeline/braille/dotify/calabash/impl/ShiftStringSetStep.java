...
public class ShiftStringSetStep extends DefaultStep implements XProcStep {
	
	@Component(
		name = "pxi:shift-string-set",
		service = { XProcStepProvider.class },
		property = { "type:String={http://www.daisy.org/ns/pipeline/xproc/internal}shift-string-set" }
	)
	public static class Provider implements XProcStepProvider {
		
		@Override
		public XProcStep newStep(XProcRuntime runtime, XAtomicStep step, XProcMonitor monitor, Map<String,String> properties) {
			return new ShiftStringSetStep(runtime, step);
		}
	}
	...
	private ShiftStringSetStep(XProcRuntime runtime, XAtomicStep step) {
		super(runtime, step);
	}
	...
	private static class ShiftStringSetTransformer extends SingleInSingleOutXMLTransformer {
		...
		void transform(BaseURIAwareXMLStreamReader reader, BaseURIAwareXMLStreamWriter output) throws TransformerException {
			...
			List<TermPair<String,Term<?>>> pendingStringSet = new ArrayList<TermPair<String,Term<?>>>();
		...
		private static class ShiftedStringSet implements FutureWriterEvent {
			...
			private List<TermPair<String,Term<?>>> stringSet;
			private boolean ready = false;
			
			private ShiftedStringSet() {
			}
			
			private void put(TermPair<String,Term<?>> stringSet) {
				if (this.stringSet == null)
					this.stringSet = new ArrayList<TermPair<String,Term<?>>>();
				this.stringSet.add(stringSet);
			}
			
			private void putAll(List<TermPair<String,Term<?>>> stringSet) {
				for (TermPair<String,Term<?>> s : stringSet)
					put(s);
			}
	...
	private static void parseStringSet(String value, List<TermPair<String,Term<?>>> appendTo) {
		PropertyValue decl = cssParser.parse(
			String.format("string-set: %s", value)).get("string-set");
		if (decl != null) {
			...
			case list_values:
				for (Term<?> t : (List<Term<?>>)decl.getValue())
					appendTo.add((TermPair<String,Term<?>>)t);
				break;
			case NONE:
				break; }}
	}
	
	private static String serializeStringSet(List<TermPair<String,Term<?>>> stringSet) {
		if (stringSet.isEmpty())
			return null;
		StringBuilder s = new StringBuilder();
		Iterator<TermPair<String,Term<?>>> it = stringSet.iterator();
		while (it.hasNext()) {
			// Relying on the fact that TermPair is a StringSet and StringSet.toString() uses
			// BrailleCssSerializer. We can not call BrailleCssSerializer directly because it is
			// private to braille-css-utils.
			s.append(it.next().toString());
			if (it.hasNext()) s.append(", "); }
		return s.toString();
	}
	
	private static final Logger logger = LoggerFactory.getLogger(ShiftStringSetStep.class);
