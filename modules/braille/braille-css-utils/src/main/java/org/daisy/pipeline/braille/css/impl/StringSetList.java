...
public class StringSetList extends AbstractList<StringSetList.StringSet> implements Term<StringSetList> {
	...
	public static class StringSet extends UnmodifiableTerm<ContentList> implements TermPair<String,ContentList> {
		...
		private StringSet(String name, ContentList value, Operator operator) {
			super();
			this.name = name;
			this.value = value;
		...
		private StringSet evaluate(Element context) {
			...
			return new StringSet(name, evaluatedValue, operator);
		}
	}
	 ...
	 */
	public static StringSetList of(BrailleCssParser parser, Context context, TermList list) {
