...
public class ParseStylesheetDefinition extends ExtensionFunctionDefinition {
	...
	private static Style parse(String argStringValue, boolean argIsAttr, QName attrName, Element context) {
		...
		if (argIsAttr) {
			if (XMLNS_CSS.equals(attrName.getNamespaceURI())) {
				...
				if ("page".equals(name)) {
					...
					Optional<Declaration> declaration = BrailleCssParser.parseDeclaration(name, argStringValue, context, false);
		...
		return new FullStyle(style);
	}

	private static final ObjectValue<Style> EMPTY = new ObjectValue<>(FullStyle.EMPTY);
