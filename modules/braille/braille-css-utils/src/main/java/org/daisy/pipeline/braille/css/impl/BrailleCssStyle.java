...
public final class BrailleCssStyle {
	...
	private static BrailleCssStyle of(RuleVolumeArea volumeArea) {
		...
		for (Rule<?> r : volumeArea)
			if (r instanceof Declaration)
				...
				style.add(BrailleCssStyle.of((RulePage)r));
			else
				throw new RuntimeException("coding error");
	...
	private static BrailleCssStyle of(RuleVolume volume, boolean relative) {
		...
		for (Rule<?> r : volume)
			if (r instanceof Declaration)
				...
				style.add(BrailleCssStyle.of((RuleVolumeArea)r));
			else
				throw new RuntimeException("coding error");
	 ...
	 */
	private static BrailleCssStyle of(AnyAtRule rule) {
		Builder style = new Builder();
		for (Rule<?> r : rule)
			if (r instanceof Declaration)
				style.add((Declaration)r);
			else if (r instanceof AnyAtRule)
				style.add("@" + ((AnyAtRule)r).getName(), BrailleCssStyle.of((AnyAtRule)r));
			else
				throw new RuntimeException("coding error");
		return style.build();
	}
	...
	static BrailleCssStyle of(InlineStyle inlineStyle, Context context) {
		...
		for (RuleBlock<?> rule : inlineStyle) {
			if (rule instanceof RuleMainBlock)
				...
				Builder decls = new Builder(); {
					for (Rule<?> r : (RuleRelativeBlock)rule)
						if (r instanceof Declaration)
							...
							decls.add(BrailleCssStyle.of((RulePage)r));
						else
							throw new RuntimeException("coding error");
				}
				...
				style.add(BrailleCssStyle.of((RulePage)rule));
			else if (rule instanceof RuleVolume)
				style.add(BrailleCssStyle.of((RuleVolume)rule));
				...
				style.add(BrailleCssStyle.of((RuleHyphenationResource)rule));
				...
				style.add(BrailleCssStyle.of((RuleVolumeArea)rule));
			else if (rule instanceof RuleRelativePage)
				style.add(BrailleCssStyle.of((RuleRelativePage)rule));
			else if (rule instanceof RuleRelativeVolume)
				style.add(BrailleCssStyle.of((RuleRelativeVolume)rule));
			else if (rule instanceof RuleRelativeHyphenationResource)
				style.add(BrailleCssStyle.of((RuleRelativeHyphenationResource)rule));
			else if (rule instanceof AnyAtRule)
				style.add("@" + ((AnyAtRule)rule).getName(),
				          BrailleCssStyle.of((AnyAtRule)rule));
			else
				throw new RuntimeException("coding error");
		}
		return style.build();
