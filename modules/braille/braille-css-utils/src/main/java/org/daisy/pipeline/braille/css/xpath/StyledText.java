...
public interface StyledText {
	...
	public static Optional<Style> getStyle(CSSStyledText t) {
		...
		return Optional.of(new FullStyle(BrailleCssStyle.of(s, Context.ELEMENT)));
	}
	...
	public static CSSStyledText of(String text, Optional<Object> style, Optional<String> language, Map<String,String> textAttributes) {
		...
		if (style != null && style.isPresent()) {
			...
			if (o instanceof Style) {
				SimpleInlineStyle s; {
					try {
						if (!(o instanceof FullStyle))
							throw new IllegalArgumentException("Unexpected style argument");
						if (((FullStyle)o).style == null)
							s = SimpleInlineStyle.EMPTY;
						else
							s = ((FullStyle)o).style.asSimpleInlineStyle(true); // this will raise a UnsupportedOperationException
