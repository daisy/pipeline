...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" version="2.0"
	...
	<xsl:variable name="empty-style" as="item()" select="css:parse-stylesheet(())"/>
	...
	<xsl:template name="expand-pseudo-elements">
		...
		<xsl:choose>
			<xsl:when test="exists($before-style) or exists($after-style)">
				<xsl:variable name="style" as="item()" select="s:remove(s:remove($style,'&amp;::before'),'&amp;::after')"/>
	...
	<xsl:template name="expand-content-property" as="node()*">
		...
		<xsl:if test="exists($content)">
			<xsl:for-each-group select="s:iterator($content)"
			                    group-adjacent="matches(string(.),'^[''&quot;]')">
				<xsl:choose>
					<xsl:when test="current-grouping-key()">
						<xsl:value-of select="string-join(for $s in current-group() return css:parse-string(string($s))/@value,'')"/>
					</xsl:when>
					<xsl:otherwise>
						<_>
							<xsl:sequence select="css:style-attribute(
							                        s:add($empty-style,'content',s:merge(current-group())))"/>
							<xsl:text> </xsl:text>
						</_>
					</xsl:otherwise>
				</xsl:choose>
