...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" version="2.0"
	...
	<xsl:template mode="identify-blocks" match="/*">
		<xsl:next-match>
			<xsl:with-param name="source-style" tunnel="yes" select="$empty-style"/>
			<xsl:with-param name="source-language" tunnel="yes" select="()"/>
			<xsl:with-param name="result-style" tunnel="yes" select="$empty-style"/>
	...
	<xsl:template name="translate-style" as="item()">
		...
		<xsl:variable name="style" as="item()*">
			<xsl:for-each select="s:iterator($style)">
				<xsl:choose>
	...
	<xsl:template mode="treewalk" match="text()">
		...
		<xsl:variable name="style" as="attribute()?" select="for $s in $translated-style return
		                                                     css:style-attribute(s:toString($s,$result-style))"/>
		<xsl:variable name="lang" as="attribute()?">
			<xsl:for-each select="$translated-language[not(.=$result-language)]">
				<xsl:attribute name="xml:lang" select="."/>
			</xsl:for-each>
