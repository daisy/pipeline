...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" version="3.0"
	...
	<xsl:template name="collapse-pseudo-elements" as="item()?">
		...
		<xsl:variable name="style" as="item()?"
		              select="if (exists($before-style)) then s:add($style,'&amp;::before',$before-style) else $style"/>
			...
			<xsl:iterate select="reverse($after-style)">
				...
				<xsl:next-iteration>
					<xsl:with-param name="nested" select="if (exists($nested))
					                                      then s:add($nested,'&amp;::after',$style)
					                                      else $style"/>
				</xsl:next-iteration>
			</xsl:iterate>
		</xsl:variable>
		<xsl:variable name="style" as="item()?"
		              select="if (exists($after-style)) then s:add($style,'&amp;::after',$after-style) else $style"/>
