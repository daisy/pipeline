    ...
    <xsl:template name="obfl:generate-layout-master">
        ...
        <xsl:variable name="page-counter-name" as="xs:string">
            <xsl:variable name="counter-increment" as="element(css:counter-set)*"
                          select="s:toXml(s:iterator($page-stylesheet)[s:property(.)='counter-increment'])
                                  /css:counter-set"/>
            <xsl:choose>
        ...
        <layout-master name="{$name}" duplex="{$duplex}"
            ...
            <xsl:if test="exists(s:iterator($right-page-stylesheet))">
                <!--
            ...
            <xsl:if test="exists(s:iterator($left-page-stylesheet))">
