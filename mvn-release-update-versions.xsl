...
<xsl:stylesheet version="2.0"
	...
	<xsl:template match="/project/parent/version|
	                     /project/dependencyManagement/dependencies/dependency[string(scope)='import']/version">
		<xsl:param name="internal-artifacts" tunnel="yes"/>
		<xsl:choose>
			<xsl:when test="$internal-artifacts[string(groupId)=string(current()/parent::*/groupId) and
			                                    string(artifactId)=string(current()/parent::*/artifactId)]">
				<xsl:copy>
					<xsl:value-of select="$internal-artifacts[string(groupId)=string(current()/parent::*/groupId) and
					                                          string(artifactId)=string(current()/parent::*/artifactId)]
					                      /version"/>
				</xsl:copy>
			</xsl:when>
	...
	<xsl:template match="/project/modules/comment()[matches(normalize-space(.),'^\s?&lt;module\s?&gt;(.+)&lt;/module\s?&gt;\s?$')]">
		<xsl:param name="module" tunnel="yes" required="yes"/>
		<xsl:variable name="submodule" select="string-join((
		                                         if ($module='.') then () else $module,
		                                         replace(normalize-space(.),'^\s?&lt;module\s?&gt;(.+)&lt;/module\s?&gt;\s?$','$1')
		                                         ),'/')"/>
		<xsl:variable name="submodule-pom" select="document(resolve-uri(concat($submodule,'/pom.xml'),$BASE))"/>
		<xsl:result-document href="{concat($submodule,'/',$OUTPUT_FILENAME)}" method="xml">
			<xsl:text>&#x0A;</xsl:text>
